...
Hit Render_World::Closest_Intersection(const Ray& ray)
{
  ...
  Hit close_hit;
 close_hit.dist = std::numeric_limits<double>::max();
 close_hit.object = NULL;
	for(unsigned int i = 0; i < objects.size(); i++){
	Hit hit = objects[i]->Intersection(ray, objects[i]->number_parts);
	if(hit.dist < close_hit.dist && hit.dist >= small_t ){
	close_hit.dist = hit.dist;
	close_hit.object = objects[i];
